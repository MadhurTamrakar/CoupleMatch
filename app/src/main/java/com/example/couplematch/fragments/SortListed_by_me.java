...
public class SortListed_by_me extends Fragment {
    ...
    public SortListed_by_me() {
    }

    public static SortListed_by_me newInstance(String param1, String param2) {
        SortListed_by_me fragment = new SortListed_by_me ();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        return inflater.inflate (R.layout.fragment_sort_listed_by_me, container, false);
    }
    ...
    private void sortlist(String user_id) {
        ...
        Call<SortlistByMe> call = apiService.get_sort_by_me (user_id);
        call.enqueue (new Callback<SortlistByMe> () {
            @SuppressLint("NotifyDataSetChanged")
            @Override
            public void onResponse(Call<SortlistByMe> call, Response<SortlistByMe> response) {
                if (response.isSuccessful ()) {
                    if (response.body ().getShortData () != null) {
                        progressDialog.dismiss ();
                        mail_box.setVisibility (View.GONE);
                        textData.setVisibility (View.GONE);
                        shortData = response.body ().getShortData ();
                        adapter = new sortlistAdapter (getActivity (), shortData);
...
//                        Toast.makeText (getActivity (), "No Content", Toast.LENGTH_SHORT).show ();
            ...
            @Override
            public void onFailure(Call<SortlistByMe> call, Throwable t) {
